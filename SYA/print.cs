        ...
        public static void PrintPageSearch(object sender, PrintPageEventArgs e, DataGridView dataGridViewSearch)
        {
            ...
            if (selectedRow != null)
            {
                ...
                if (tagNumber.Length > 1)
                {
                    ...
                    {
                        print_helper.onlyGross((selectedRow.Cells["gross"].Value ?? "0").ToString(),e);
                    }
                    else
                    {
                        print_helper.gross((selectedRow.Cells["gross"].Value ?? "0").ToString(),e);
                        print_helper.net((selectedRow.Cells["net"].Value ?? "0").ToString(),e);
                       
                    }
                    print_helper.image(e);
                    ...
                    if (result == "BANGAL")
                    {
                        print_helper.textBelowLogo((selectedRow.Cells[12].Value ?? "0").ToString(), e);
                    }
                    else
                    {
                        print_helper.textBelowLogo("YAMUNA", e);
                    }
                   print_helper.quality((selectedRow.Cells["itemdesc"].Value ?? "0").ToString().Split('-')[0].Trim() ?? "0",e);
                    print_helper.QR(tagNumber,e);
                    string labour = "0";
                    if (selectedRow.Cells["labour"].Value.ToString() != "0")
                    {
                        labour = (selectedRow.Cells["labour"].Value ?? "-").ToString();
                        print_helper.labour(labour,e);
                    }
                    else if ((selectedRow.Cells["wholeLabour"].Value ?? "-").ToString() != "0")
                    {
                        labour = (selectedRow.Cells["wholeLabour"].Value ?? "-").ToString();
                        print_helper.wholeLabour(labour,e);
                    }
                    if ((selectedRow.Cells["other"].Value ?? "0").ToString() != "0")
                    {
                        print_helper.other((selectedRow.Cells["other"].Value ?? "0").ToString(),e);
                    }
                    ...
                    if (length >= 10)
                    {
                        ...
                        e.Graphics.DrawString(secondPart, new Font("Arial", (float)6, FontStyle.Bold), brush, new RectangleF((float)174, (float)32, (float)37, (float)7), new StringFormat() { Alignment = StringAlignment.Center, LineAlignment = StringAlignment.Center });
                        e.Graphics.DrawString(firstPart, new Font("Arial", (float)6
                            , FontStyle.Bold), brush, new RectangleF((float)174, (float)42, (float)37, (float)7), new StringFormat() { Alignment = StringAlignment.Center, LineAlignment = StringAlignment.Center });
                       // e.Graphics.DrawString(huid1, new Font("Arial", (float)7, FontStyle.Bold), brush, new RectangleF((float)113.5, (float)29, (float)56.5, (float)10), new StringFormat() { Alignment = StringAlignment.Center, LineAlignment = StringAlignment.Center });
                       // e.Graphics.DrawString(huid2, new Font("Arial", (float)7, FontStyle.Bold), brush, new RectangleF((float)113.5, (float)38, (float)56.5, (float)12), new StringFormat() { Alignment = StringAlignment.Center, LineAlignment = StringAlignment.Center });
                    }
                    else
                    {
                        e.Graphics.DrawString(tagNumber, new Font("Arial", (float)5, FontStyle.Bold), brush, new RectangleF((float)174, (float)38, (float)37, (float)7), new StringFormat() { Alignment = StringAlignment.Center, LineAlignment = StringAlignment.Center });
