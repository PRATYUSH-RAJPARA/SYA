    ...
    public class QUERY
    {
        ...
        public static void GetSummaryData()
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                check = helper.FetchDataTableFromSYADataBaseSummary("SELECT * FROM Summary WHERE CO_YEAR = '" + CO_YEAR + "' AND MONTH = '" + row["unique_months"].ToString() + "' AND ITEM='GOLD'");
                ...
                check = helper.FetchDataTableFromSYADataBaseSummary("SELECT * FROM Summary WHERE CO_YEAR = '" + CO_YEAR + "' AND MONTH = '" + row["unique_months"].ToString() + "' AND ITEM='SILVER'");
                if (check.Rows.Count > 0)
                {
                    MessageBox.Show("123");
                    updateSQL();
...
";
    ...
    SET 
        ...
        ITM_NWT = " + ITM_NWT + @",
        ITM_RATE = " + ITM_RATE + @",
        _916_COUNT = " + _916_COUNT + @",
        _18K_COUNT = " + _18K_COUNT + @",
        _20K_COUNT = " + _20K_COUNT + @",
        LBR_RATE = " + LBR_RATE + @",
        LBR_AMT = " + LBR_AMT + @",
        OTH_AMT = " + OTH_AMT + @"
